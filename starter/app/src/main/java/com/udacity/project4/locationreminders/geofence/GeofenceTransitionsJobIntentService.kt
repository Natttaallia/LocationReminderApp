...
class GeofenceTransitionsJobIntentService : JobIntentService(), CoroutineScope {
    ...
    override fun onHandleWork(intent: Intent) {
        ...
        if (geoEvent.hasError()) {
            Log.e(this::javaClass.name, "Error code: ${geoEvent.errorCode}")
            return
        }

        if (geoEvent.geofenceTransition == Geofence.GEOFENCE_TRANSITION_ENTER) {
            sendNotification(geoEvent.triggeringGeofences)
            Log.d(this::javaClass.name, applicationContext.getString(R.string.geofence_entered))
        }
    }

    private fun sendNotification(triggeringGeofences: List<Geofence>) {
        for (geo in triggeringGeofences) {
            sendGeofence(geo.requestId)
        }
    }
